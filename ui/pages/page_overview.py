#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
概览仪表盘 - Page 1
"""

import flet as ft
from ui.theme import *


def build_page(data: dict, theme: dict = None) -> list:
    """返回页面组件列表"""
    if theme is None:
        theme = load_theme()

    # 颜色从theme获取
    GRAY = theme.get("secondary_text", "#888888")
    GREEN = theme.get("green", "#34a853")
    YELLOW = theme.get("yellow", "#f9ab00")
    RED = theme.get("red", "#ea4335")
    ACCENT = theme.get("accent", "#4a9eff")
    CARD_BG = theme.get("card_bg", "#1B2838")
    DARK_BG = theme.get("dark_bg", "#0A0F1A")
    TEXT_PRI = theme.get("primary_text", "#ffffff")

    # 风险级别
    risks = data.get('risks', [])
    high_risks = [r for r in risks if r[0] == '高']
    med_risks  = [r for r in risks if r[0] == '中']
    risk_level = '高' if high_risks else '中' if med_risks else '低'
    risk_color = RED if risk_level == '高' else YELLOW if risk_level == '中' else GREEN

    # 内存百分比
    mem_pct = float(data.get('mem_pct', 0))
    mem_color = GREEN if mem_pct < 70 else YELLOW if mem_pct < 90 else RED

    # CPU负载
    cpu_load = float(data.get('cpu_load', 0) or 0)
    cpu_color = GREEN if cpu_load < 70 else YELLOW if cpu_load < 90 else RED

    cards = []

    # 概览卡片 - 主机名/OS/风险等级
    cards.append(ft.Container(
        content=ft.Column([
            ft.Row([
                ft.Column([
                    ft.Text(data.get('hostname', 'N/A'), size=24, weight=ft.FontWeight.W_700, color=TEXT_PRI),
                    ft.Text(data.get('os', 'N/A'), size=13, color=GRAY),
                ], spacing=4),
                ft.Container(expand=True),
                ft.Container(
                    padding=ft.Padding.only(left=16, right=16, top=8, bottom=8),
                    border_radius=12,
                    bgcolor=risk_color,
                    content=ft.Text(f'⚠️ {risk_level}风险', size=16, weight=ft.FontWeight.W_700, color=TEXT_PRI),
                ),
            ]),
            ft.Divider(height=20, color=CARD_BG),
            ft.Row([
                ft.Column([ft.Text('运行时间', size=11, color=GRAY), ft.Text(data.get('uptime', 'N/A'), size=13, color=TEXT_PRI)], spacing=2),
                ft.Container(expand=True),
                ft.Column([ft.Text('当前用户', size=11, color=GRAY), ft.Text(data.get('user', 'N/A'), size=13, color=TEXT_PRI)], spacing=2),
                ft.Container(expand=True),
                ft.Column([ft.Text('许可证', size=11, color=GRAY), ft.Text(f"{data.get('license_status','N/A')} ({data.get('license_type','N/A')})", size=13, color=TEXT_PRI)], spacing=2),
            ]),
        ], spacing=0),
        padding=20, border_radius=16, bgcolor=CARD_BG,
    ))

    # CPU/内存卡片（双列）
    cards.append(ft.Row([
        ft.Container(
            expand=1, padding=16, border_radius=16, bgcolor=CARD_BG,
            content=ft.Column([
                ft.Text('💻 CPU负载', size=13, color=GRAY),
                ft.Text(f"{cpu_load:.0f}%", size=28, weight=ft.FontWeight.W_700, color=cpu_color),
                ft.Container(
                    height=8, border_radius=4,
                    bgcolor="#2A3A4A",
                    content=ft.Stack([
                        ft.Container(width=max(cpu_load, 2), height=8, border_radius=4, bgcolor=cpu_color),
                    ]),
                ),
                ft.Text(data.get('cpu_name', 'N/A')[:40], size=10, color=GRAY),
            ], spacing=6),
        ),
        ft.Container(width=10),
        ft.Container(
            expand=1, padding=16, border_radius=16, bgcolor=CARD_BG,
            content=ft.Column([
                ft.Text('🧠 内存', size=13, color=GRAY),
                ft.Text(f"{mem_pct:.1f}%", size=28, weight=ft.FontWeight.W_700, color=mem_color),
                ft.Container(
                    height=8, border_radius=4,
                    bgcolor="#2A3A4A",
                    content=ft.Stack([
                        ft.Container(width=max(mem_pct, 2), height=8, border_radius=4, bgcolor=mem_color),
                    ]),
                ),
                ft.Text(f"{data.get('mem_used', 0):.1f} / {data.get('mem_total', 0):.1f} GB", size=10, color=GRAY),
            ], spacing=6),
        ),
    ]))

    # 磁盘卡片
    disk_items = []
    for i, disk in enumerate(data.get('disks', [])):
        pct = float(disk.get('pct', 0))
        color = GREEN if pct < 70 else YELLOW if pct < 90 else RED
        disk_items.append(ft.Container(
            padding=8, border_radius=8, bgcolor=CARD_BG, margin=ft.Margin.only(bottom=6),
            content=ft.Column([
                ft.Row([
                    ft.Column([ft.Text(disk.get('drive',''), size=14, weight=ft.FontWeight.W_600, color=TEXT_PRI)], spacing=0),
                    ft.Column([ft.Text(disk.get('label','N/A'), size=10, color=GRAY), ft.Text(f"{disk.get('free','N/A')}GB 可用 / {disk.get('total','N/A')}GB", size=10, color=GRAY)], spacing=0, tight=True),
                    ft.Container(expand=True),
                    ft.Text(f"{pct:.1f}%", size=14, weight=ft.FontWeight.W_700, color=color),
                ], spacing=8),
                ft.Container(
                    height=6, border_radius=3, bgcolor="#2A3A4A",
                    content=ft.Stack([
                        ft.Container(width=max(int(pct/100*200), 4), height=6, border_radius=3, bgcolor=color),
                    ]),
                ),
            ], spacing=4),
        ))
        # 细分隔线（非最后一项）
        if i < len(data.get('disks', [])) - 1:
            disk_items.append(ft.Container(height=1, bgcolor="#2A3A4A", margin=ft.Margin.only(bottom=5)))

    cards.append(ft.Container(
        padding=16, border_radius=16, bgcolor=CARD_BG,
        content=ft.Column([ft.Text('💾 磁盘', size=15, weight=ft.FontWeight.W_600, color=TEXT_PRI)] + disk_items, spacing=4),
    ))

    # 风险告警列表
    risk_items = []
    for i, (level, desc) in enumerate(risks):
        color = RED if level == '高' else YELLOW
        risk_items.append(ft.Container(
            padding=10, border_radius=8, bgcolor=CARD_BG, margin=ft.Margin.only(bottom=6),
            content=ft.Row([
                ft.Container(
                    width=8, height=8, border_radius=4, bgcolor=color,
                ),
                ft.Text(desc, size=12, color=TEXT_PRI),
                ft.Container(expand=True),
                ft.Container(
                    padding=ft.Padding.only(left=8, right=8, top=3, bottom=3),
                    border_radius=6, bgcolor=color,
                    content=ft.Text(level, size=10, weight=ft.FontWeight.W_700, color=TEXT_PRI),
                ),
            ], spacing=8),
        ))
        if i < len(risks) - 1:
            risk_items.append(ft.Container(height=1, bgcolor="#2A3A4A", margin=ft.Margin.only(bottom=5)))

    cards.append(ft.Container(
        padding=16, border_radius=16, bgcolor=CARD_BG,
        content=ft.Column(
            [ft.Text(f'🚨 风险告警 ({len(risks)})', size=15, weight=ft.FontWeight.W_600, color=TEXT_PRI)] +
            (risk_items if risk_items else [ft.Text('未发现风险项 ✓', size=12, color=GREEN)]),
            spacing=4,
        ),
    ))

    return cards